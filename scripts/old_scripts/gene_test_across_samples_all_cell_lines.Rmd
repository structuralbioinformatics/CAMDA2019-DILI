---
title: "CMap Drug Safety Challenge: Gene test across samples of all cell lines"
...
Among samples from Most-DILI-Concern drugs, there are `r length(gct_most@cid)` samples. These samples have different doses and times:

```{r}
table(gct_most@cdesc$pert_idose, gct_most@cdesc$pert_itime)
```

Same happens with No-DILI-Concern drugs, with `r length(gct_no@cid)` samples in different doses and times:

```{r}
table(gct_no@cdesc$pert_idose, gct_no@cdesc$pert_itime)
```
...
output_table <- "/home/quim/PHD/Projects/camda/results/gene_test_all_10_24.tsv"
