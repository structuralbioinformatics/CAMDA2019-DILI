...
# Output files
output.cv.rf <- sprintf("/home/quim/PHD/Projects/camda/results/crossvalidation/correlated_samples/cv_%s_%s_smiles_top%s_%s_correlated_rf", database, type_genes, num_top, corr_threshold)
output.cv.gbm <- sprintf("/home/quim/PHD/Projects/camda/results/crossvalidation/correlated_samples/cv_%s_%s_smiles_top%s_%s_correlated_gbm", database, type_genes, num_top, corr_threshold)
output.ind.rf = sprintf("/home/quim/PHD/Projects/camda/camda_data/independent_validation/correlated_samples/JAguirre_predictions_%s_%s_smiles_top%s_%s_correlated_rf", database, type_genes, num_top, corr_threshold)
output.ind.gbm = sprintf("/home/quim/PHD/Projects/camda/camda_data/independent_validation/correlated_samples/JAguirre_predictions_%s_%s_smiles_top%s_%s_correlated_gbm", database, type_genes, num_top, corr_threshold)
output.cv.comb.rf <- sprintf("/home/quim/PHD/Projects/camda/results/crossvalidation/cv_%s_%s_smiles_comb_top%s_%s_correlated_rf", database, type_genes, num_top, corr_threshold)
output.cv.comb.gbm <- sprintf("/home/quim/PHD/Projects/camda/results/crossvalidation/cv_%s_%s_smiles_comb_top%s_%s_correlated_gbm", database, type_genes, num_top, corr_threshold)
output.single.ind.rf = sprintf("/home/quim/PHD/Projects/camda/camda_data/independent_validation/JAguirre_predictions_%s_%s_smiles_single_top%s_%s_correlated_rf", database, type_genes, num_top, corr_threshold)
output.single.ind.gbm = sprintf("/home/quim/PHD/Projects/camda/camda_data/independent_validation/JAguirre_predictions_%s_%s_smiles_single_top%s_%s_correlated_gbm", database, type_genes, num_top, corr_threshold)
output.comb.ind.rf = sprintf("/home/quim/PHD/Projects/camda/camda_data/independent_validation/JAguirre_predictions_%s_%s_smiles_comb_top%s_%s_correlated_rf", database, type_genes, num_top, corr_threshold)
output.comb.ind.gbm = sprintf("/home/quim/PHD/Projects/camda/camda_data/independent_validation/JAguirre_predictions_%s_%s_smiles_comb_top%s_%s_correlated_gbm", database, type_genes, num_top, corr_threshold)
...
for (phenotype in phenotypes){
  ...
  if (length(gene_ids)>=minimum_genes) {
    ...
    ### Combine expression and SMILES data ###
    # Combine the main datasets
    tanimoto_df$dilirank <- NULL
    tanimoto_df$severity <- NULL
    combined_data <- merge(x = expression_df, y = tanimoto_df, by = "pert_iname")
    # Combine the independent datasets
    tanimoto_ind_df$dilirank <- NULL
    tanimoto_ind_df$severity <- NULL
    combined_data_ind <- merge(x = expression_ind_df, y = tanimoto_ind_df, by = "pert_iname")
    independent.datasets.list[[length(independent.datasets.list)+1]] <- combined_data_ind
    
    ### Prepare balanced machine learning datasets ###
    datasets.list <- prepare.datasets.from.balanced.drugs(combined_data, balanced.drugs.list$drugs_testing, balanced.drugs.list$drugs_training_list, type_analysis="discrete")
    ...
    disgenet.ind.rf <- validate.classifier(combined_data_ind, 
    ...
    disgenet.ind.gbm <- validate.classifier(combined_data_ind, 
