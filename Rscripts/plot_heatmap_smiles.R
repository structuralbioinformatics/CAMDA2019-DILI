...
most_df <- data.frame(Tanimoto=tanimoto_most_val, DILIrank="Most- vs. Most-DILI-Concern")
less_df <- data.frame(Tanimoto=tanimoto_less_val, DILIrank="Less- vs. Less-DILI-Concern")
no_df <- data.frame(Tanimoto=tanimoto_no_val, DILIrank="No- vs. No-DILI-Concern")
most_no_df <- data.frame(Tanimoto=tanimoto_most_no_val, DILIrank="Most- vs. No-DILI-Concern")
most_less_df <- data.frame(Tanimoto=tanimoto_most_less_val, DILIrank="Most- vs. Less-DILI-Concern")
less_no_df <- data.frame(Tanimoto=tanimoto_less_no_val, DILIrank="Less- vs. No-DILI-Concern")
all_df <- rbind(most_df, less_df, no_df)
most_no_comparison_df <- rbind(most_df, most_no_df, no_df)

Cairo::CairoPNG(output_hist_png, dpi=300, width = 6, height = 4, units = "in") # Resolution taken from: https://www.andrewheiss.com/blog/2017/09/27/working-with-r-cairo-graphics-custom-fonts-and-ggplot/
...
Cairo::CairoPNG(output_hist2_png, dpi=300, width = 6, height = 4, units = "in") # Resolution taken from: https://www.andrewheiss.com/blog/2017/09/27/working-with-r-cairo-graphics-custom-fonts-and-ggplot/
